...
class SpriteManager(object):
    ...
    _TRANSPARENCY = ["rose.png", "plants.png", "water-lily.png"]
    ...
    def __init__(self):
        ...
        if self._INSTANCE != None:
            raise RuntimeError(
                "Cannot create more than one instance of the SpriteManager."
            )
    ...
    def getSize(self, fileName):
        spriteSize = self._SPRITE_SIZES.get(fileName, self._DEFAULT_SPRITE)

        scale = self._SCALES.get(fileName, 1)
    ...
    def getSprite(self, fileName, offset=None):
        ...
        if type(offset) == Rect or (type(offset) in [list, tuple] and len(offset) == 4):
            if type(offset) != Rect:
                offset = Rect(offset)

            if tuple(offset) not in self._surfaces[fileName]:
                self._surfaces[fileName][tuple(offset)] = self._loadRect(
                    fileName, self._surfaces[fileName]["full"], offset
                )
    ...
    def _loadImage(self, fileName, offset=None):
        ...
        fullImage = scale_by(fullImage, self._SCALES.get(fileName, 1))
        ...
        elif type(offset) == Rect or (
            type(offset) in [list, tuple] and len(offset) == 4
        ):
        ...
        # If there is no offset store the entire image
            ...
            raise TypeError(
                f"Offset must be of type Rect, List, Tuple, or None. Length of List/Tuple must be 2 for spriteSheet offset or 4 for arbitrary rect."
            )
    ...
    def _loadSpriteSheet(self, fileName, fullImage):
        ...
        for y in range(0, sheetDimensions[1], spriteSize[1]):
            ...
            for x in range(0, sheetDimensions[0], spriteSize[0]):
                ...
                self._surfaces[fileName][-1].append(
                    self._loadRect(fileName, fullImage, Rect((x, y), spriteSize))
                )
